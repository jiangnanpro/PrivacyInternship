...
class ShadowModelBundle(sklearn.base.BaseEstimator):
    ...
    def _transform(self, shadow_indices=None, verbose=False):
        ...
        for i in model_index_iter:
            ...
            shadow_data, shadow_data_dense2, shadow_labels = prepare_attack_data(
                shadow_model, train_data, test_data
            )

            shadow_data_array.append(shadow_data)
            shadow_data_dense2_array.append(shadow_data_dense2)
            shadow_label_array.append(shadow_labels)

        X_transformed = np.vstack(shadow_data_array).astype("float32")
        X_transformed_dense2 = np.vstack(shadow_data_dense2_array).astype("float32")
        y_transformed = np.hstack(shadow_label_array).astype("float32")
        return X_transformed, X_transformed_dense2, y_transformed
...
def prepare_attack_data(model, data_in, data_out):
    ...
    y_hat_dense2_in = get_1st_layer_output(X_in)[0]
    y_hat_dense2_out = get_1st_layer_output(X_out)[0]
    ...
    data_dense2 = np.c_[y_hat_dense2_in, y_in]
    data_dense2 = np.vstack([data_dense2, np.c_[y_hat_dense2_out, y_out]])
    
    return data, data_dense2, labels
    ...
    def predict_proba(self, X):
        ...
        for i in range(self.num_classes):
            ...
            if self.class_one_hot_coded:
                ...
                class_indices = data_indices[np.squeeze(classes) == i]
            if shadow_preds[class_indices].shape[0] == 0:
            	continue
