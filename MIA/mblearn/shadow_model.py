...
class ShadowModels:
    ...
    def __init__(
        ...
        **fit_kwargs,
        ...
        self._splits_X, self._splits_y = self._split_data(self.X, self.y, self.n_models, self.target_classes)
    ...
    def _split_data(
        ...
        for clss in classes:
            ...
            for i in range(n_splits):
                ...
                splits_y.append(split_y.reshape(-1, 1))
        ...
        # Todo: maybe we need to randomly reorder the splited datasets
        grouped_X = []
        grouped_y = []
        for split in range(n_splits):
            parts_X = []
            parts_y = []
            for part_X in class_partitions_X:
                parts_X.append(part_X[split])
            for part_y in class_partitions_y:
        ...
        splits_X = []
        splits_y = []
        for group_X in grouped_X:
            splits_X.append(np.vstack(group_X))
        for group_y in grouped_y:
            splits_y.append(np.vstack(group_y))
    ...
    def _make_model_list(learner, n) -> List:
        ...
        try:
            ...
            print("using sklearn shadow models hhh")
            pass
    ...
    def train_predict_shadows(self, **fit_kwargs):
        ...
        # TRAIN and predict
        results = []
        for model, data_subset_X, data_subset_y in tqdm_notebook(zip(self.models, self._splits_X, self._splits_y)):
            X = data_subset_X
            y = data_subset_y
