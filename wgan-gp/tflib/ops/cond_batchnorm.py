...
def Batchnorm(name, axes, inputs, is_training=None, stats_iter=None, update_moving_stats=True, fused=True, labels=None, n_labels=None):
    ...
    offset = tf.compat.v1.nn.embedding_lookup(offset_m, labels)
    scale = tf.compat.v1.nn.embedding_lookup(scale_m, labels)
    result = tf.compat.v1.nn.batch_normalization(inputs, mean, var, offset[:,:,None,None], scale[:,:,None,None], 1e-5)
    return result