...
def lpips(input0, input1, normalize=True, model='net-lin', net='alex', version=0.1):
    ...
    batch_shape = tf.compat.v1.shape(input0)[:-3]
    input0 = tf.compat.v1.reshape(input0, tf.compat.v1.concat([[-1], tf.shape(input0)[-3:]], axis=0))
    input1 = tf.compat.v1.reshape(input1, tf.compat.v1.concat([[-1], tf.shape(input1)[-3:]], axis=0))
    # NHWC to NCHW
    input0 = tf.compat.v1.transpose(input0, [0, 3, 1, 2])
    input1 = tf.compat.v1.transpose(input1, [0, 3, 1, 2])
    ...
    if distance.shape.ndims == 4:
        distance = tf.compat.v1.squeeze(distance, axis=[-3, -2, -1])
    # reshape the leading dimensions
    distance = tf.compat.v1.reshape(distance, batch_shape)
    return distance
...
def get_lpips_scores(imgset0,imgset1,max_batch_size=64):
    ...
    image0_ph = tf.compat.v1.placeholder(tf.compat.v1.float32)
    image1_ph = tf.compat.v1.placeholder(tf.compat.v1.float32)
