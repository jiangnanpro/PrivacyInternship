...
def get_activations(images, sess, batch_size=50, verbose=False):
    ...
    if images.shape[-1] != 3:
      images_exp = np.expand_dims(images,axis=-1)
      images = np.concatenate((images_exp,images_exp,images_exp),axis=-1)
...
def calculate_mean_cov(path, inception_path, low_profile=False):
    ''' Calculate the mean and cov of a given path. '''
    inception_path = check_or_download_inception(inception_path)
...
if __name__ == "__main__":
    ...
    if len(args.path) == 2:
        ...
        mean, cov = calculate_mean_cov(args.path, args.inception, low_profile=args.lowprofile)
